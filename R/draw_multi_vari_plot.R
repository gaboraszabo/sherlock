...
draw_multi_vari_plot <- function(data, response, factor_1, factor_2, factor_3, plot_means = FALSE) {
  ...
  if (missing(factor_3)) {
    ...
    multi_vari_chart <- multi_vari_tbl %>%
      ...
      ggh4x::facet_nested(rows = vars(), cols = vars(!!factor_2_expr)) +
      ggplot2::guides(y.sec = "axis") +
      ggplot2::theme_element +
      ggplot2::labs(
        title = "Multi-Vari Chart",
        subtitle = str_glue("{as_label(factor_1_expr)} by {as_label(factor_2_expr)}"),
        x = str_glue("{as_label(factor_1_expr)}"),
        y = str_glue("{as_label(response_expr)}")
      )

    if (plot_means) {
      multi_vari_chart <- multi_vari_chart +
        ggplot2::geom_point(color = "grey30", size = 1.5, alpha = 0.4) +
        ggplot2::geom_point(data = mean_tbl, aes(!!factor_1_expr, mean), color = "#40506e", size = 3, alpha = 0.7) +
        ggplot2::geom_line(data = mean_tbl, aes(!!factor_1_expr, mean, group = 1), color = "#40506e", alpha = 0.7) +
        ggplot2::labs(caption  = str_glue("Blue data points represent averages for factor {as_label(factor_1_expr)}"))
    } else {
      multi_vari_chart <- multi_vari_chart +
        ggplot2::geom_point(size = 2.5, alpha = 0.6) +
        ggplot2::geom_line(aes(group = !!factor_1_expr), size = 0.7, alpha = 0.6)
    }
    ...
    if (plot_means) {
      ...
      multi_vari_chart <- multi_vari_tbl %>%
        ...
        ggplot2::geom_point(color = "grey30", size = 1.5, alpha = 0.4) +
        ggplot2::geom_point(data = mean_tbl, aes(!!factor_2_expr, mean), color = "#40506e", size = 3, alpha = 0.7) +
        ggplot2::geom_line(data = mean_tbl, aes(!!factor_2_expr, mean, group = 1), color = "#40506e", alpha = 0.7) +

        ggh4x::facet_nested(rows = vars(), cols = vars(!!factor_3_expr)) +
        ggplot2::guides(y.sec = "axis") +

        theme_element +
        ggplot2::labs(
          title    = "Multi-Vari Chart",
          subtitle = str_glue("{as_label(factor_1_expr)} by {as_label(factor_2_expr)} by {as_label(factor_3_expr)}"),
          x        = str_glue("{as_label(factor_2_expr)}"),
          y        = str_glue("{as_label(response_expr)}"),
          caption  = str_glue("Blue data points represent averages for factor {as_label(factor_2_expr) }
      ...
      multi_vari_chart <- multi_vari_tbl %>%
        ...
        ggh4x::facet_nested(rows = vars(), cols = vars(!!factor_3_expr, !!factor_2_expr)) +
        ggplot2::guides(y.sec = "axis") +
